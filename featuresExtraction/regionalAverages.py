...
"""
def __extractRegionalAverages(poly_data, section_distance):
...
def regionalAverages(reader):
    ...
    for idx in range(len(sections__x_distances)):
        ...
        section_regional_averages = __extractRegionalAverages(poly_data, sections__x_distances[idx])

        # Appending the signal of the current section to the nd signal
        regional_averages["p"][:,idx] = section_regional_averages["p"]
        regional_averages["U"][:,idx] = section_regional_averages["U"]

    # Converting the numpy array to lists
    regional_averages["p"] = regional_averages["p"].tolist()
    regional_averages["U"] = regional_averages["U"].tolist()
